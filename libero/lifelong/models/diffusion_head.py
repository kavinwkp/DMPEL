...
class DiffusionPolicy(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.obs_dim = obs_dim              # 64
        self.action_dim = act_dim           # 7
        self.obs_horizon = obs_horizon      # 10
        self.pred_horizon = pred_horizon    # 10
    ...
    def _update(
        ...
        obs_seq: torch.Tensor,
        action_seq: Optional[torch.Tensor],
    ) -> Optional[torch.Tensor]:  # , Optional[torch.Tensor], Dict[str, float]]:
        ...
        if self.policy_type == "cnn":
            action_seq = torch.cat(
                (
                    torch.zeros_like(action_seq[:, : self.pad_before]),
                    action_seq,
                    torch.zeros_like(action_seq[:, : self.pad_after]),
                ),
                dim=1,
            )
        # naction = self.normalize_act_data(action_seq).to(self.device)   # (bs, 10, 7)
        naction = action_seq   # (bs, 10, 7)
        # nobs = self.normalize_obs_data(obs_seq).to(self.device)     # (bs, 10, 64)
        nobs = obs_seq     # (bs, 10, 64)
        B = nobs.shape[0]

        # observation as FiLM conditioning
        # (B, obs_horizon, obs_dim)
        obs_cond = nobs[:, : self.obs_horizon, :]   # (bs, 10, 64)

        # sample noise to add to actions
        noise = torch.randn(naction.shape, device=self.device)  # (bs, 10, 7)
        ...
        noisy_actions = self.noise_scheduler.add_noise(naction, noise, timesteps)   # x_0 -> x_t
...
if __name__ == '__main__':
    ...
    x = torch.randn(bs, 1, obs_dim)

    action = torch.randn(bs, 10, act_dim)

    output = policy(x, action)  # (B, 1, 256) {"noise_pred", "noise"}
