...
from libero.lifelong.models.modules.adapter import LoRAqkv, L2MLoRAqkv, L2MLoRA, MoELoRA, MoELoRAqkv
###############################################################################
#
...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        fullft: Optional[bool] = True,
        lora_cfg: Optional[EasyDict] = None,
        **kwargs
    ):
        super().__init__()
    ...
    def forward(self, x, mask=None):
        for layer_idx, (att_norm, att, ff_norm, ff) in enumerate(self.layers):
            if mask is not None:
                ...
                x = x + drop_path(att(x=att_norm(x)))
