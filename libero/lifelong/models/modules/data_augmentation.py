...
class AffineAug(nn.Module):
    ...
    def __init__(
        ...
        self.affine = torchvision.transforms.RandomAffine(
    ...
    def forward(self, x):
        if self.training and np.random.rand() > self.epsilon:
            bsz, l, img_c, img_h, img_w = x.shape
            x = x.reshape(bsz * l, img_c, img_h, img_w)
            out = self.affine(x)
            out = out.reshape(bsz, l, img_c, img_h, img_w)
        else:
            out = x
        return out
