...
class BasePolicy(nn.Module, metaclass=PolicyMeta):
    def __init__(self, cfg, shape_meta):
        ...
        policy_cfg.affine_aug.network_kwargs["input_shape"] = shape_meta[
            "all_shapes"
        ][cfg.data.obs.modality.rgb[0]]
        affine_aug = eval(policy_cfg.affine_aug.network)(**policy_cfg.affine_aug.network_kwargs)
        self.img_aug = DataAugGroup((color_aug, affine_aug, translation_aug))
    ...
    def preprocess_input(self, data, train_mode=True, augmentation=True):
        if train_mode:  # apply augmentation
            if self.cfg.train.use_augmentation and augmentation:
            ...
            if self.cfg.text_tokens_or_embeddings == "tokens":
                data["task_emb"] = {key: value.squeeze(1) for key, value in data["task_emb"].items()}
            elif self.cfg.text_tokens_or_embeddings == "embeddings":
                data["task_emb"] = data["task_emb"].squeeze(1)
            elif self.cfg.text_tokens_or_embeddings == "hidden_states":
                data["task_emb"] = data["task_emb"].squeeze(1)
        return data
