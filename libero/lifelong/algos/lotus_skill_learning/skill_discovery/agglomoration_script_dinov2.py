...
def agglomoration_func(cfg, modality_str, dataset_name_list, lifelong_dataset_name_list):
    ...
    for dataset_name in dataset_name_list:
        dataset_idx += 1
        dataset_name_path = "/public/home/group_luoping/leiyuheng/dataset/datasets/" + dataset_name + ".hdf5"
        h5py_file = h5py.File(dataset_name_path, "r")
        demo_num = len(h5py_file['data'].keys())
        embedding_hdf5_path = f"/public/home/group_luoping/leiyuheng/dataset/datasets/lotus/{cfg.exp_name}/repr/{dataset_name}/embedding_{modality_str}_{cfg.repr.z_dim}.hdf5"
        embedding_h5py_f = h5py.File(embedding_hdf5_path, "r")
        with open(f"/public/home/group_luoping/leiyuheng/dataset/datasets/lotus/{cfg.exp_name}/skill_classification/trees/{dataset_name}_trees_{modality_str}_{cfg.agglomoration.footprint}_{cfg.agglomoration.dist}.pkl", "rb") as f:
            trees = pickle.load(f)
    ...
    #     dataset_name = dataset_name_list[dataset_idx]
    #     os.makedirs(f"/public/home/group_luoping/leiyuheng/dataset/datasets/lotus/{cfg.exp_name}/skill_data_vis/{dataset_name}", exist_ok=True)
    #     plt.savefig(f"/public/home/group_luoping/leiyuheng/dataset/datasets/lotus/{cfg.exp_name}/skill_data_vis/{dataset_name}/{modality_str}_{cfg.agglomoration.footprint}_{cfg.agglomoration.dist}_{cfg.agglomoration.segment_footprint}_K{K}_{cfg.agglomoration.affinity}.png")
    #     plt.close()
    ...
    #         plt.ylabel("No. Demo")
    #         ax.axis('off')
    #         plt.savefig(f"/public/home/group_luoping/leiyuheng/dataset/datasets/lotus/{cfg.exp_name}/skill_data_vis/{dataset_name}/{modality_str}_{cfg.agglomoration.footprint}_{cfg.agglomoration.dist}_{cfg.agglomoration.segment_footprint}_K{K}_{cfg.agglomoration.affinity}_image.png")
    #         plt.close()
    ...
    for dataset_idx in ep_subtasks_seq: 
        ...
        os.makedirs(f"/public/home/group_luoping/leiyuheng/dataset/datasets/lotus/{cfg.exp_name}/skill_data/{dataset_category}", exist_ok=True)
        subtask_file_name = f"/public/home/group_luoping/leiyuheng/dataset/datasets/lotus/{cfg.exp_name}/skill_data/{dataset_name}_subtasks_{modality_str}_{cfg.repr.z_dim}_{cfg.agglomoration.footprint}_{cfg.agglomoration.dist}_{cfg.agglomoration.segment_footprint}_K{K}_{cfg.agglomoration.affinity}.hdf5"
    ...
    with h5py.File(f"/public/home/group_luoping/leiyuheng/dataset/datasets/lotus/{cfg.exp_name}/skill_data/saved_feature_data.hdf5", 'w') as hf:
    ...
    cfg.exp_name='dinov2_libero_object_image_only'
    # cfg.exp_name='dinov2_libero_spatial_image_only'
    # cfg.exp_name='dinov2_libero_goal_image_only'
    # cfg.exp_name='dinov2_libero_10_image_only'
    for lifelong_dataset_name in lifelong_dataset_name_list:
        ...
        seg_start = [x[4] for x in X]
        seg_end = [x[5] for x in X]
        save_exp_name = f"{cfg.exp_name}/task_{len(dataset_name_list)}"
        with h5py.File(f"/public/home/group_luoping/leiyuheng/dataset/datasets/lotus/{save_exp_name}/skill_data/saved_feature_data.hdf5", 'w') as hf:
            hf.create_dataset('embeddings', data=np.stack(embeddings))
            hf.create_dataset('cluster_labels', data=cluster_labels)
            hf.create_dataset('task_ids', data=task_ids)
            hf.create_dataset('demo_indices', data=demo_indices)
...
def assign_or_create_new_cluster(data, ep_idx, X, distance_fn, dataset_idx):
    ...
    if nearest_distance > max_distance_between_centers: # and max([label for _, label, _, _ in X]) < 19:
...
def visualize_tsne(tsne_results, labels, title, colors_list, name, cfg, dataset_name, new_dataset_idx, xlim=None, ylim=None):
    ...
    os.makedirs(f"/public/home/group_luoping/leiyuheng/dataset/datasets/lotus/{save_exp_name}/skill_data_vis/{dataset_name}", exist_ok=True)
    plt.savefig(f"/public/home/group_luoping/leiyuheng/dataset/datasets/lotus/{save_exp_name}/skill_data_vis/{dataset_name}/debug_{name}.png")
    # plt.close()
...
def add_new_data(cfg, modality_str, old_ep_subtasks_seq, X, old_dataset_name_list, dataset_name):
    ...
    #     os.makedirs(f"/public/home/group_luoping/leiyuheng/dataset/datasets/lotus/{save_exp_name}/skill_data_vis/{dataset_name}", exist_ok=True)
    #     plt.savefig(f"/public/home/group_luoping/leiyuheng/dataset/datasets/lotus/{save_exp_name}/skill_data_vis/{dataset_name}/{modality_str}_{cfg.agglomoration.footprint}_{cfg.agglomoration.dist}_{cfg.agglomoration.segment_footprint}_K{K}_{cfg.agglomoration.affinity}.png")
    #     plt.close()
    ...
    #         plt.ylabel("No. Demo")
    #         ax.axis('off')
    #         plt.savefig(f"/public/home/group_luoping/leiyuheng/dataset/datasets/lotus/{save_exp_name}/skill_data_vis/{dataset_name}/{modality_str}_{cfg.agglomoration.footprint}_{cfg.agglomoration.dist}_{cfg.agglomoration.segment_footprint}_K{K}_{cfg.agglomoration.affinity}_image.png")
    #         plt.close()
    ...
    for dataset_idx in all_ep_subtasks_seq: 
        ...
        dataset_category = dataset_name.split("/")[0]
        save_exp_name = f"{cfg.exp_name}/task_{len(dataset_name_list)}"
        os.makedirs(f"/public/home/group_luoping/leiyuheng/dataset/datasets/lotus/{save_exp_name}/skill_data/{dataset_category}", exist_ok=True)
        subtask_file_name = f"/public/home/group_luoping/leiyuheng/dataset/datasets/lotus/{save_exp_name}/skill_data/{dataset_name}_subtasks_{modality_str}_{cfg.repr.z_dim}_{cfg.agglomoration.footprint}_{cfg.agglomoration.dist}_{cfg.agglomoration.segment_footprint}_K{K}_{cfg.agglomoration.affinity}.hdf5"
        print(subtask_file_name)
        subtask_file = h5py.File(subtask_file_name, "w")
...
@hydra.main(config_path="/public/home/group_luoping/leiyuheng/project/pearl/libero/configs/lifelong/lotus", config_name="default", version_base=None)
def main(hydra_cfg):
    
    yaml_config = OmegaConf.to_yaml(hydra_cfg, resolve=True)
    cfg = EasyDict(yaml.safe_load(yaml_config))
    cfg.repr.z_dim=1536
    cfg.agglomoration.dist='cos' 
    cfg.agglomoration.footprint='global_pooling'
    cfg.exp_name='dinov2_libero_90_image_only'
    cfg.modality_str='dinov2_agentview_eye_in_hand'
    cfg.agglomoration.segment_scale=1
    cfg.agglomoration.min_len_thresh=30 
    cfg.agglomoration.K=10 # optimal K with highest silhouette value
    cfg.agglomoration.scale=0.01
    ...
    base_dataset_name_list = ["libero_90/" + task + "_demo" for task in libero_task_map["libero_90"]]

    lifelong_dataset_name_list = ["libero_object/" + task + "_demo" for task in libero_task_map["libero_object"]]
    # lifelong_dataset_name_list = ["libero_spatial/" + task + "_demo" for task in libero_task_map["libero_spatial"]]
    # lifelong_dataset_name_list += ["libero_goal/" + task + "_demo" for task in libero_task_map["libero_goal"]]
    # lifelong_dataset_name_list += ["libero_10/" + task + "_demo" for task in libero_task_map["libero_10"]]

    
    print(base_dataset_name_list)
    print(lifelong_dataset_name_list)
